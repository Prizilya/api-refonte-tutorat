...
def getUnclosedProposalsBot
  request_object = OpenConnectBdd.query('SELECT m.intitule AS matiere, p.id_proposition as id_proposition, p.id_createur as id_createur, p.id_matiere as id_matiere FROM proposition p JOIN matiere m ON p.id_matiere=m.id_matiere JOIN proposition_promo po ON p.id_proposition=po.id_proposition JOIN personne pe ON p.id_createur=pe.id_personne')
